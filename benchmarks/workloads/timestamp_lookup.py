...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="lerobot-timestamp-benchmark-"
    ) as directory:
        ...
        for targets in args.targets:
            ...
            results = run(cli, "\n".join(sql))
            checksums = [
                row[2:] for row in results if row[0] in ("parquet", "materialized")
            ]
            ...
            saving = (
                scenario["parquet"]["warm_median_seconds"]
                - scenario["materialized"]["warm_median_seconds"]
            )
            scenario["break_even_reuses_estimate"] = (
                build["latency"] / saving if saving > 0 else None
            )
