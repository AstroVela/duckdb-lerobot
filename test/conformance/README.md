...
access policy through unsupported demuxers. It disguises a DASH manifest as
an MP4 shard and points it at generated media outside the allowed dataset
directory, then at a loopback HTTP server. Frames, targets and windows must reject these references with both
...
The C++ `lerobot_video_producer_test` additionally constructs a valid MOV with
an external data reference from the same H.264 fixture. A native FFmpeg read
must resolve that reference and return media bytes; the decoder's I/O adapter
must instead report a permission error. External tracks are enabled only in
this test, because MOV normally skips them before calling `io_open`. This
checks the rejecting callback independently of the MP4/MOV format restriction
and runs in both FFmpeg CI jobs, with leak detection in the sanitizer job.
