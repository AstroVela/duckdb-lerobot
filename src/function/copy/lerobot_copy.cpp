...
bool HasIntegerExtrema(const LerobotFeature &feature) {
	return LerobotHasIntegerExtrema(feature.dtype);
}

LogicalType StatExtremaLeafType(const LerobotFeature &feature) {
	return LerobotStatExtremaLeafType(feature.dtype);
}
...
vector<LerobotFeature> ParseUserFeatures(ClientContext &context, const string &features_json) {
	...
	while (true) {
		...
		for (idx_t row = 0; row < chunk->size(); row++) {
			...
			if (StringUtil::CIEquals(feature.name, "task") || StringUtil::CIEquals(feature.name, "timestamp") ||
			    StringUtil::CIEquals(feature.name, "frame_index") ||
			    StringUtil::CIEquals(feature.name, "episode_index") || StringUtil::CIEquals(feature.name, "index") ||
			    StringUtil::CIEquals(feature.name, "task_index")) {
			...
			feature.storage_type = LerobotFeatureStorageType(feature.dtype, feature.shape);
			feature.output_type = LerobotFeatureOutputType(feature.dtype, feature.shape);
...
LerobotFeature DefaultFeature(const string &name, const string &dtype) {
	...
	feature.dtype = dtype;
	feature.shape = {1};
	feature.storage_type = LerobotDtypeToLogicalType(dtype);
...
struct LerobotCopyBindData : public FunctionData {
	...
	idx_t fps = 0;
	idx_t chunks_size = 0;
	double data_file_size_mb = 0;
	double video_file_size_mb = 0;
	idx_t metadata_buffer_size = 0;
...
void AppendStatColumns(LerobotCopyBindData &bind, const LerobotFeature &feature) {
	...
	auto extrema_type = LerobotNestedListType(StatExtremaLeafType(feature), stat_shape.size());
	auto value_type = LerobotNestedListType(LogicalType::DOUBLE, stat_shape.size());
...
unique_ptr<FunctionData> LerobotCopyBind(ClientContext &context, CopyFunctionBindInput &input,
	...
	result->features_json = config.features_json;
	...
	ParseLerobotCopyOptionalConfig(context, input, config);
	result->chunks_size = config.chunks_size;
	result->data_file_size_mb = config.data_file_size_mb;
	result->video_file_size_mb = config.video_file_size_mb;
	result->metadata_buffer_size = config.metadata_buffer_size;
...
CopyFunction LerobotCopyFunction::Create() {
	...
	function.copy_options = LerobotCopyOptionDefinitions;
